...
const modificarServicio = async () => {
    ...
    try {
        await servicioStore.modificarServicio(idServicio.value, nuevoServicio);
        error.value = "";
        ...
        error.value = "Error al modificar el servicio.";
...
onMounted(async () => {
    idServicio.value = Number(route.params.id);
    await servicioStore.obtenerServicio(idServicio.value);
    if (!servicioStore.servicio) return;

    nombre.value = servicioStore.servicio.nombre;
    descripcion.value = servicioStore.servicio.descripcion;
    precio.value = servicioStore.servicio.precio;
    duracion_estimada.value = servicioStore.servicio.duracion_estimada;
});
</script>
...
<template>
    <div class="page">
        <div class="card">
            <h2 class="title">
                <Icon icon="mdi:briefcase-edit" class="icon" /> Modificar
                Servicio
            </h2>
